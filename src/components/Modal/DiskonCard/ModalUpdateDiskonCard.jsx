...
const ModalUpdateDiskonCard = ({ open, setOpen, getData, data }) => {
  const initialValues = {
      ...
      onSubmit={async (values, { setSubmitting, resetForm }) => {
        try {
          ...
          const res = await API.put(
            `${ENDPOINTS.UPDATE_IMAGE_DISKON}/${data?.id}`,
            formData,
            config
          );
          ...
          setTimeout(() => {
            setSubmitting(false);
            toastSuccess(`Diskon Card berhasil diupdate`);
            resetForm();
            getData();
          }, 400);
        } catch (error) {
...
export default ModalUpdateDiskonCard;
